    ...
    def _save_snapshot_files(self):
        ...
        for idx, snap_boundary in enumerate(self.snap_boundaries):
            ...
            if idx == 0:
                ...
                finetune_train = self.data_df[(self.n_train_batches + snap_boundary - gap) * self.batch_size:(self.n_train_batches + snap_boundary) * self.batch_size].values.astype(np.int64)
                print('finetune_train shape: {}'.format(finetune_train.shape))
